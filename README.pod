...
B<fizzy> is a bare minimal command-line tool to filter input records based on
user queries and print selected ones to standard output.

B<fizzy> exposes no limitations on input record length, but restricts queries
to 32 bytes. The good news is that latter limit is practically unreachable
because queries tend to be a lot shorter than this. However, if needed,
B<fuzzy-filter-matched> function (default bound to B<SPACE>) can be used to
set a filter to currently matched records, excluding all non-matching ones from
further queries.

B<fizzy> has UTF-8 support but only in the ASCII range supports
case-insensitive matches (e.g. "o" matches "O").

=head1 INTERFACE

B<fizzy> uses a fullscreen interface that looks like the following:
...
Set readline application name. Default: B<Fizzy>.
...
Set custom bindings in .inputrc
  ...
  $if Fizzy # Refer to -n to change name.
  +: fizzy-emit-one
  $endif
...
Report bugs to L<https://gitlab.com/zsugabubus/fizzy>.
